...
import { Building2, CalendarDays, Check, ChevronDown, Download, Layers3, MapPin, Megaphone, MoreHorizontal, Pencil, Plus, Search } from 'lucide-react';
import { getHotelById, listEventCategories, listEvents, listGroupEventLocations, resolveLoc, type Loc } from '@aidahos/db';
...
const STATUS_LABELS: Record<EventStatus, readonly [string, string]> = {
  scheduled: ['Planlandı', 'Scheduled'], live: ['Devam Ediyor', 'Live'], full: ['Dolu', 'Full'],
  completed: ['Tamamlandı', 'Completed'], draft: ['Taslak', 'Draft'], cancelled: ['İptal', 'Cancelled'],
};
const STATUS_CLASSES: Record<EventStatus, string> = {
  scheduled: 'info', live: 'ok', full: 'warn', completed: 'mute', draft: 'mute', cancelled: 'err',
};

function FilterChip({ icon, label }: { icon: React.ReactNode; label: string }) {
  return <button className="fchip events-filter-chip" type="button">{icon}{label}<span className="chev"><ChevronDown size={14} /></span></button>;
}

function fmtDate(d: Date | null, lang: Lang): string {
  if (!d) return '—';
  return new Intl.DateTimeFormat(lang === 'tr' ? 'tr-TR' : 'en-US', { day: '2-digit', month: 'short' }).format(d);
}
...
export default async function EventsListPage({ params }: { params: Promise<{ hotelId: string }> }) {
  ...
  const [rows, cats, locs] = groupId
    ? await Promise.all([listEvents(groupId), listEventCategories(groupId), listGroupEventLocations(groupId)])
    : [[], [], []];
    ...
    <div className="events-list fade-in">
      <div className="page-hero events-hero">
        <div>
          ...
          <p className="page-hero__sub">{L(['Tüm etkinlikleri görüntüleyin, filtreleyin ve yönetin.', 'View, filter and manage all events.'], lang)}</p>
        </div>
        <div className="page-hero__actions">
          <button className="btn btn--ghost" type="button"><Download />{L(['Dışa Aktar', 'Export'], lang)}</button>
          <Link className="btn btn--primary" href={`${base}/new`}><Plus />{L(['Yeni Etkinlik', 'New Event'], lang)}</Link>
        </div>
      </div>
      ...
      <div className="filterbar events-filterbar">
        ...
        <label className="searchmini events-search"><Search size={15} /><input placeholder={L(['Etkinlik ara…', 'Search events…'], lang)} /></label>
      </div>
      ...
      <section className="card events-list-card">
        <div className="card__body events-table-wrap">
          <table className="events-table table">
            ...
            <tbody>
              {rows.length === 0 ? (
                <tr><td colSpan={7} style={{ color: 'var(--text-3)', padding: '24px 4px', textAlign: 'center' }}>{L(['Henüz etkinlik yok. “Yeni Etkinlik” ile ekleyin.', 'No events yet. Use “New Event” to add one.'], lang)}</td></tr>
              ) : rows.map((ev) => {
                  ...
                  <tr key={ev.id}>
                    <td>
                      <div className="table__name events-table__event">
                        <div className="events-table__icon" style={{ background: `color-mix(in srgb, ${color} 16%, transparent)`, color }}><CalendarDays size={17} /></div>
                        <div className="events-table__namecopy">
                          <div className="events-table__title">{rl(ev.name as Loc) || L(['(adsız)', '(untitled)'], lang)}</div>
                          {loc ? <div className="cell-sub">{rl(loc)}</div> : null}
                        </div>
                      </div>
                    </td>
                    <td>{cat ? <span className="cat"><span className="cat__dot" style={{ background: color }} />{rl(cat.name)}</span> : <span className="cell-sub">—</span>}</td>
                    <td className="events-table__muted">{fmtDate(start, lang)}</td>
                    <td className="mono events-table__muted">{start ? `${fmtTime(start)}${endD ? `–${fmtTime(endD)}` : ''}` : '—'}</td>
                    <td><div className="events-capacity"><MiniBar pct={0} color={color} maxWidth={64} /><span>0/{ev.capacity}</span></div></td>
                    <td><span className={`badge badge--${STATUS_CLASSES[status]}`}><span className="ico-dot" />{L(STATUS_LABELS[status], lang)}</span></td>
                    <td>
                      <div className="rowact events-row-actions">
                        <Link href={`${base}/${ev.id}/edit`} aria-label={L(['Düzenle', 'Edit'], lang)}><Pencil /></Link>
                        <Link href={`${base}/notify`} aria-label={L(['Bildirim gönder', 'Send notification'], lang)}><Megaphone /></Link>
        ...
        <div className="pager events-pager">
          <div className="pager__info">{L([`${rows.length} etkinlik`, `${rows.length} events`], lang)}</div>
          <div className="pager__nums">
            <button type="button">{L(['Önceki', 'Prev'], lang)}</button>
            <button className="on" type="button">1</button>
