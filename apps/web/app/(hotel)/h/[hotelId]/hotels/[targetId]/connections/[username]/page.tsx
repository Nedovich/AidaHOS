...
export default async function AdminConnectionDetail({
  params,
}: {
  params: Promise<{ hotelId: string; targetId: string; username: string }>;
}) {
  const { hotelId, targetId, username: raw } = await params;
  ...
  return (
    <>
      <div className="header__crumb" style={{ marginBottom: 14, display: 'flex', alignItems: 'center', gap: 7 }}>
        <Link href={base} className="row-link" style={{ color: 'var(--text-3)' }}>{L(['Oteller', 'Hotels'], lang)}</Link>
        <span>›</span>
        <Link href={`${base}/${hotel.id}`} className="row-link" style={{ color: 'var(--text-3)' }}>{hotel.name}</Link>
        <span>›</span>
        <Link href={`${base}/${hotel.id}?tab=connections`} className="row-link" style={{ color: 'var(--text-3)' }}>{L(['Bağlantılar', 'Connections'], lang)}</Link>
        <span>›</span>
      ...
      <div className="acct-detail-head">
        ...
        <Link className="btn btn--ghost btn--sm" href={`${base}/${hotel.id}?tab=connections`}>
